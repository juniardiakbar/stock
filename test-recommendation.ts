...
async function runTest() {
  ...
  console.log(`Bandarmology Score: ${item.suggestion.bandarmologyScore}/100 (${item.suggestion.bandarmologyStatus})`);
  ...
  console.log(`Stop Loss: Rp ${item.tradingPlan.stopLoss.price.toLocaleString()} (${item.tradingPlan.stopLoss.reason})`);
  console.log(`TP1: Rp ${item.tradingPlan.takeProfit1.price.toLocaleString()} (+${item.tradingPlan.takeProfit1.percentFromCurrent.toFixed(1)}%)`);
  console.log(`TP2: Rp ${item.tradingPlan.takeProfit2.price.toLocaleString()} (+${item.tradingPlan.takeProfit2.percentFromCurrent.toFixed(1)}%)`);
  console.log(`TP3: Rp ${item.tradingPlan.takeProfit3.price.toLocaleString()} (+${item.tradingPlan.takeProfit3.percentFromCurrent.toFixed(1)}%)`);
  ...
  if (item.tradingPlan.addZones.length > 0) {
    ...
    item.tradingPlan.addZones.forEach((zone, i) => {
      console.log(`  ${i + 1}. ${zone.lots} lots @ Rp ${zone.price.toLocaleString()} (${zone.priority}) - ${zone.reason}`);
    });
  }
  ...
  item.tradingPlan.sellStrategy.forEach((step, i) => {
    ...
    console.log(`  ${i + 1}. ${step.triggerCondition}: ${lots} @ Rp ${step.price.toLocaleString()} - ${step.reason}`);
  });
