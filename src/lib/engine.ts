...
export function calculatePortfolio(
  ...
  const grouped = transactions.reduce((acc, t) => {
    ...
    return acc;
  }, {} as Record<string, { totalLots: number; totalCost: number; transactions: Transaction[] }>);
  ...
  return Object.entries(grouped).map(([symbol, data]) => {
    ...
    const allocationPercent = settings.totalCapital > 0 ? (marketValue / settings.totalCapital) * 100 : 0;

    // Generate suggestion and trading plan
    const suggestion = generateSuggestion(stock, data.totalLots, avgPrice, unrealizedPLPercent, settings);
    const tradingPlan = generateTradingPlan(stock, data.totalLots, avgPrice, unrealizedPLPercent, settings, availableCapital);
...
function generateSuggestion(
  ...
  const { rsi, ma20, ma5, trend, support, resistance, pricePosition, atr } = stock.indicators;
  const { score: bandarmologyScore, status: bandarmologyStatus, warningLevel, signals, pattern } = stock.bandarmology;
  const currentPrice = stock.currentPrice;

  const warningFlags: string[] = [];
  let action: ActionSuggestion['action'] = 'HOLD';
  let urgency: ActionSuggestion['urgency'] = 'NONE';
  let reason = 'Menunggu sinyal lebih jelas.';

  // Risk tolerance modifiers
  const riskMod = settings.riskTolerance === 'CONSERVATIVE' ? 0.8 : settings.riskTolerance === 'AGGRESSIVE' ? 1.2 : 1;
  ...
  if (warningLevel === 'DANGER') {
    warningFlags.push('BAHAYA: Bandar distribusi aktif!');
  } else if (warningLevel === 'CAUTION') {
    warningFlags.push('WASPADA: Ada tanda-tanda distribusi');
  }

  if (bandarmologyStatus === 'DISTRIBUTION' || bandarmologyStatus === 'STRONG_DISTRIBUTION') {
  ...
  if (bandarmologyStatus === 'STRONG_DISTRIBUTION') {
    action = 'SELL';
    urgency = 'IMMEDIATE';
    reason = `DISTRIBUSI KUAT terdeteksi (Score: ${bandarmologyScore}). Keluar sekarang!`;
  }
  // PRIORITY 2: Distribution + Loss = Cut position
  else if (bandarmologyStatus === 'DISTRIBUTION' && unrealizedPLPercent < 0) {
    action = 'REDUCE';
    urgency = 'SOON';
    reason = `Distribusi + Floating Loss (${unrealizedPLPercent.toFixed(1)}%). Kurangi posisi.`;
  }
  // PRIORITY 3: Hard Stop Loss
  else if (unrealizedPLPercent <= -settings.stopLossTarget) {
    action = 'SELL';
    urgency = 'IMMEDIATE';
    reason = `Stop Loss hit (${unrealizedPLPercent.toFixed(1)}%). Cut loss sekarang!`;
  }
  ...
  // TP1 level = half of target (e.g., 10% if target is 20%)
    ...
    action = 'TAKE_PROFIT';
    urgency = 'IMMEDIATE';
    reason = `TARGET HIT! Profit +${unrealizedPLPercent.toFixed(1)}%. Saatnya ambil profit!`;
  }
  else if (unrealizedPLPercent >= settings.takeProfitTarget * 0.5 && unrealizedPLPercent > 0) {
    // Price has reached TP1 level (half of target, e.g., 10% if target is 20%)
    action = 'TAKE_PROFIT';
    urgency = 'SOON';
    reason = `TP1 tercapai (+${unrealizedPLPercent.toFixed(1)}%). Pertimbangkan jual sebagian.`;
  }
  else if (rsi >= (settings.riskTolerance === 'CONSERVATIVE' ? 70 : 80) && pricePosition > 85) {
    action = 'TAKE_PROFIT';
    urgency = 'SOON';
    reason = `RSI Overbought (${rsi.toFixed(0)}) + Near resistance. Pertimbangkan jual.`;
  }
  else if (pattern === 'DISTRIBUTION_CEILING' && unrealizedPLPercent > 5) {
    action = 'TAKE_PROFIT';
    urgency = 'WATCH';
    reason = `Ceiling pattern - ambil profit sebelum reversal.`;
  }
  // PRIORITY 5: Buy opportunities (Strong Accumulation)
  else if (bandarmologyStatus === 'STRONG_ACCUMULATION' && trend !== 'DOWN') {
    action = 'STRONG_BUY';
    urgency = 'SOON';
    reason = `AKUMULASI KUAT (Score: ${bandarmologyScore}). Bandar sedang collect!`;
  }
  else if (bandarmologyStatus === 'ACCUMULATION' && rsi < 60 && currentPrice <= ma20 * 1.03) {
    action = 'BUY';
    urgency = 'WATCH';
    reason = `Akumulasi + Near MA20 support. Good entry.`;
  }
  else if (pattern === 'SHAKEOUT') {
    action = 'STRONG_BUY';
    urgency = 'SOON';
    reason = `Shakeout recovery! Bandar goyang retail - buy the dip.`;
  }
  else if (pattern === 'BREAKOUT') {
    action = 'BUY';
    urgency = 'SOON';
    reason = `Volume breakout! Momentum bullish.`;
  }
  // PRIORITY 6: Reduce on bad conditions even with small profit
  else if (warningLevel === 'DANGER' && unrealizedPLPercent > 0) {
    action = 'REDUCE';
    urgency = 'SOON';
    reason = `Warning level DANGER dengan profit. Amankan sebagian.`;
  }
  ...
  const isNearExit = currentPrice <= avgPrice * (1 - settings.stopLossTarget / 100 * 0.8) ||
                     unrealizedPLPercent >= settings.takeProfitTarget * 0.8;
...
function generateTradingPlan(
  ...
  const defaultPlan: TradingPlan = {
    ...
    maxDownside: 0,
    maxUpside: 0,
    immediateAction: 'Data tidak tersedia',
    shortTermPlan: 'Menunggu data',
    notes: ''
  };
  ...
  const { score: bandarmologyScore, status: bandarmologyStatus, warningLevel, pattern } = stock.bandarmology;
  ...
  if (bandarmologyStatus === 'STRONG_DISTRIBUTION' || unrealizedPLPercent <= -settings.stopLossTarget) {
    positionHealth = 'DANGER';
  } else if (bandarmologyStatus === 'DISTRIBUTION' || warningLevel === 'DANGER' || unrealizedPLPercent < -5) {
    positionHealth = 'WARNING';
  ...
  const stopLoss: PriceLevel = {
    ...
    reason: stopLossPrice === technicalStop
      ? `Support level Rp${Math.round(support).toLocaleString()}`
      : stopLossPrice === atrStop
        ? `2x ATR dari harga saat ini`
        : `${settings.stopLossTarget}% dari harga beli`
  };
  ...
  const tp3Target = avgPrice * (1 + settings.takeProfitTarget / 100 * tp3Multiplier); // Extended (e.g., 30%)
  ...
  const tp3Price = Math.max(tp3Target, currentPrice * 1.10);

  const takeProfit1: PriceLevel = {
    price: Math.round(tp1Price),
    percentFromCurrent: ((tp1Price - currentPrice) / currentPrice) * 100,
    reason: resistance > currentPrice && tp1Price >= resistance * 0.98
      ? `Near resistance Rp${Math.round(resistance).toLocaleString()}`
      : `Target pertama (${((tp1Price - currentPrice) / currentPrice * 100).toFixed(1)}%)`
  };

  const takeProfit2: PriceLevel = {
    price: Math.round(tp2Price),
    percentFromCurrent: ((tp2Price - currentPrice) / currentPrice) * 100,
    reason: `Target utama ${settings.takeProfitTarget}%`
  };

  const takeProfit3: PriceLevel = {
    price: Math.round(tp3Price),
    percentFromCurrent: ((tp3Price - currentPrice) / currentPrice) * 100,
    reason: `Target extended (${((tp3Price - currentPrice) / currentPrice * 100).toFixed(1)}%)`
  };
  ...
  const maxAllocationAmount = (settings.maxAllocationPerStock / 100) * settings.totalCapital;
  ...
  if (remainingAllocation > 0 && availableCapital > 0) {
    ...
    if (support > 0 && support < currentPrice * 0.95) {
      ...
      const lotsAtSupport = Math.floor(buyableAmount * 0.4 / (supportPrice * 100));
      if (lotsAtSupport > 0) {
        addZones.push({
          price: supportPrice,
          lots: lotsAtSupport,
    ...
    if (bandarmologyScore >= 75) {
      ...
      if (lotsNow > 0) {
        addZones.push({
          ...
          lots: lotsNow,
          reason: `Strong accumulation - entry sekarang`,
          priority: 'HIGH'
        });
      }
    }
    ...
    if (pattern === 'BREAKOUT' || (bandarmologyScore >= 70 && rsi < 70)) {
      ...
      const lotsAtBreakout = Math.floor(buyableAmount * 0.2 / (breakoutPrice * 100));
      if (lotsAtBreakout > 0) {
        addZones.push({
          price: breakoutPrice,
          lots: lotsAtBreakout,
  ...
  if (positionHealth === 'DANGER') {
    // Emergency exit
    sellStrategy.push({
      triggerCondition: 'SEGERA - Kondisi bahaya',
      price: Math.round(currentPrice),
      lotsToSell: 'ALL',
      percentOfPosition: 100,
      reason: 'Cut loss / exit distribusi'
    });
  } else if (positionHealth === 'WARNING') {
    // Partial exit
    sellStrategy.push({
      triggerCondition: 'Warning aktif - kurangi exposure',
      price: Math.round(currentPrice),
      lotsToSell: Math.ceil(totalLots * 0.5),
      percentOfPosition: 50,
      reason: 'Reduce risk - jual setengah posisi'
    });
    sellStrategy.push({
      triggerCondition: `Jika turun ke Rp${stopLoss.price.toLocaleString()}`,
      price: stopLoss.price,
      lotsToSell: 'ALL',
      percentOfPosition: 100,
      reason: 'Stop loss - jual semua sisa'
    });
  } else {
    ...
    if (tp1Lots > 0) {
      sellStrategy.push({
        triggerCondition: `TP1: Harga mencapai Rp${takeProfit1.price.toLocaleString()}`,
        price: takeProfit1.price,
        lotsToSell: tp1Lots,
        percentOfPosition: 30,
        reason: 'Take partial profit - amankan 30%'
      });
    }
    if (tp2Lots > 0) {
      sellStrategy.push({
        triggerCondition: `TP2: Harga mencapai Rp${takeProfit2.price.toLocaleString()}`,
        price: takeProfit2.price,
        lotsToSell: tp2Lots,
        percentOfPosition: 40,
        reason: 'Target utama - jual 40%'
      });
    }
    if (tp3Lots > 0) {
      sellStrategy.push({
        triggerCondition: `TP3: Harga mencapai Rp${takeProfit3.price.toLocaleString()}`,
        price: takeProfit3.price,
        lotsToSell: tp3Lots,
        percentOfPosition: 30,
        reason: 'Extended target - jual sisa'
      });
    }

    // Always have stop loss
    sellStrategy.push({
      triggerCondition: `STOP LOSS: Jika turun ke Rp${stopLoss.price.toLocaleString()}`,
      price: stopLoss.price,
      lotsToSell: 'ALL',
      percentOfPosition: 100,
      reason: 'Cut loss - proteksi modal'
    });
  }
  ...
  if (positionHealth === 'DANGER') {
    immediateAction = `JUAL SEMUA ${totalLots} lot sekarang di harga Rp${Math.round(currentPrice).toLocaleString()}`;
    shortTermPlan = 'Keluar dari posisi ini. Tunggu konfirmasi trend baru sebelum masuk lagi.';
    notes = 'Posisi dalam kondisi bahaya. Prioritaskan proteksi modal.';
  } else if (positionHealth === 'WARNING') {
    immediateAction = `Pertimbangkan jual ${Math.ceil(totalLots * 0.5)} lot (50%) di Rp${Math.round(currentPrice).toLocaleString()}`;
    shortTermPlan = `Monitor ketat. Stop loss di Rp${stopLoss.price.toLocaleString()}`;
    notes = 'Ada warning signals. Kurangi exposure dan monitor.';
  } else if (positionHealth === 'EXCELLENT') {
    immediateAction = 'HOLD - posisi sehat';
    if (addZones.length > 0 && addZones[0].priority === 'HIGH') {
      immediateAction = `Boleh tambah ${addZones[0].lots} lot di Rp${addZones[0].price.toLocaleString()}`;
    }
    shortTermPlan = `Target TP1: Rp${takeProfit1.price.toLocaleString()}, TP2: Rp${takeProfit2.price.toLocaleString()}`;
    notes = 'Posisi bagus dengan bandarmology positif. Ikuti plan.';
  } else {
    immediateAction = 'HOLD - tunggu sinyal lebih jelas';
    shortTermPlan = `Watch support Rp${Math.round(support).toLocaleString()}, resistance Rp${Math.round(resistance).toLocaleString()}`;
    if (addZones.length > 0) {
      notes = `Ada zona beli: ${addZones.map(z => `Rp${z.price.toLocaleString()} (${z.lots} lot)`).join(', ')}`;
    }
  }
